...
PaDEL = function(SMILES_list) {
  ...
  for (i in 1:dim(SMILES_list)[1]) {
    ...
    descs_json = paste(output_string[1][[1]],
                       output_string[2][[1]],
                       output_string[3][[1]],
                       output_string[4][[1]],
                       sep = "")
...
PaDEL_original = function(standards) {
  ...
  write_delim(standards %>% select(SMILES) %>% na.omit(),
              "SMILES.smi",
              col_names = FALSE)
  ...
  command_final = paste(
    ...
    "-2d",
    sep = " "
  ) #makes text for command prompt
  javaOutput = system(command_final, intern = TRUE) #goes into command prompt
